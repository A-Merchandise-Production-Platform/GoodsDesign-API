...
import { FactoryOrderStatus, FactoryStatus, OrderStatus, Roles } from '@prisma/client';
...
async checkAndReassignExpiredOrders(): Promise<void> {
    try {
      console.log('Running cron job: checkAndReassignExpiredOrders');
      ...
      console.log(`Found ${expiredOrders.length} expired factory orders`);
  
      for (const order of expiredOrders) {
        try {
        //   await this.reassignOrderToNextFactory(order.id);
          console.log(`Successfully reassigned factory order ${order.id}`);
        } catch (error) {
          console.error(`Failed to reassign factory order ${order.id}:`, error);
      ...
      console.error('Error in checkAndReassignExpiredOrders cron job:', error);
    }
  }
  ...
  async sendReminderNotifications(): Promise<void> {
    try {
      console.log('Running cron job: sendReminderNotifications');
      ...
      console.log(`Found ${almostExpiredOrders.length} factory orders nearing acceptance deadline`);
      ...
      console.log(`Found ${almostDueOrders.length} factory orders nearing completion deadline`);
      ...
      console.error('Error in sendReminderNotifications cron job:', error);
    }
  }
  ...
  async updateOverdueOrders(): Promise<void> {
    try {
      console.log('Running cron job: updateOverdueOrders');
      ...
      console.log(`Found ${overdueOrders.length} overdue factory orders`);
      ...
      console.error('Error in updateOverdueOrders cron job:', error);
    }
  }
  ...
  async updateFactoryCapacity(): Promise<void> {
    try {
      console.log('Running cron job: updateFactoryCapacity');
      ...
      console.log(`Updating capacity for ${factories.length} factories`);
      ...
      console.error('Error in updateFactoryCapacity cron job:', error);
    }
  }
  ...
  async generateFactoryPerformanceReport(): Promise<void> {
    try {
      console.log('Running cron job: generateFactoryPerformanceReport');
      ...
      console.error('Error in generateFactoryPerformanceReport cron job:', error);
    }
  }
  ...
  async createQualityCheckTasksForCompletedOrders(): Promise<void> {
    try {
      console.log('Running cron job: createQualityCheckTasksForCompletedOrders');
    ...
    //   console.log(`Found ${completedOrders.length} completed orders needing quality check tasks`);
    ...
    //       console.log(`Created quality check task for factory order ${order.id}`);
    //     } catch (error) {
    //       console.error(`Failed to create quality check task for factory order ${order.id}:`, error);
    //     }
    //   }
    } catch (error) {
      console.error('Error in createQualityCheckTasksForCompletedOrders cron job:', error);
    }
  }
  ...
  async assignPendingTasksToStaff(): Promise<void> {
    try {
      console.log('Running cron job: assignPendingTasksToStaff');
      ...
      if (availableStaff.length === 0) {
        console.log('No available staff to assign tasks');
        return;
      }
      ...
      console.error('Error in assignPendingTasksToStaff cron job:', error);
    }
  }
  ...
  async checkAndUpdateExpiredTasks(): Promise<void> {
    try {
      console.log('Running cron job: checkAndUpdateExpiredTasks');
      ...
      console.error('Error in checkAndUpdateExpiredTasks cron job:', error);
    }
  }
  ...
  async processCompletedOrdersForDelivery(): Promise<void> {
    try {
      console.log('Running cron job: processCompletedOrdersForDelivery');
      ...
      console.error('Error in processCompletedOrdersForDelivery cron job:', error);
    }
  }
}