...
export async function seedOrders(prisma: PrismaClient) {
    ...
    // Get used voucher
    const usedVoucher = await prisma.voucher.findFirst({
        where: { id: "voucher005" }
    })

    if (!customer || !factories.length || !designs.length) {
        console.log(
            "⚠️ Missing required data (customers, factories, or designs). Skipping order seeding."
        )
        return
    }
    ...
    const calculateDesignPrice = (designId: string) => {
        ...
        const designPositions = designPositionsData.designPositions.filter(
            (dp) => dp.designId === designId
        )
    ...
    const subTotal = completedOrderPrice * 4
    let totalPrice = subTotal

    // Apply voucher discount if found
    if (usedVoucher) {
        if (usedVoucher.type === "PERCENTAGE") {
            totalPrice = subTotal - subTotal * (usedVoucher.value / 100)
        } else if (usedVoucher.type === "FIXED_VALUE") {
            totalPrice = subTotal - usedVoucher.value
        }
    }
    ...
    const completedOrder = await prisma.order.create({
        data: {
            ...
            shippingPrice: 50000,
            totalItems: 4,
            voucherId: usedVoucher ? usedVoucher.id : undefined,
